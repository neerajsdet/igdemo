@motor
Feature: Verify Motor Insurance getPremium api, endpoint:/protection/motor-proposal/api/v1/policies
  ...
  Scenario: Validate the motor policy api
    ...
    Given set the request base url "motor_url" and endpoint "motor_getPremium_endpoint"
    Given set the request headers, query params
      | h:X-PIBPL-USER-SSO-TOKEN | h:X-PIBPL-USER-WALLET-TOKEN          | qp: proposal|
