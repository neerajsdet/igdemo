...
#    Then perform the "POST" api call 1 times and validate response code 200
    ...
    Given set the request headers, query params and payload with json file name "oms_verify.json"
      ...
      | application/json | sso_token   | 1235272705               | 155                                        |
    Then perform the "POST" api call 5 times and validate response code 200
    ...
    Given set the request headers, query params and payload with json file name "oms_verify.json"
      ...
      | application/json | sso_token   | 1235272705               | iosapp    | 1                  | 155                                        |
    Then perform the "POST" api call 5 times and validate response code 200 and get the value of "ORDER_ID,MID,native_withdraw_details.txnToken,TXN_AMOUNT" from response and save in global data with key "order_id,mid,txn_token,txn_amount"
    ...
    Given set the request headers, query params
      ...
      | sso_token   | order_id    |
    Then perform the "GET" api call 5 times and validate response code 200 and get the value of "my_ins_details[0].premium_details.item_id" from response and save in global data with key "cart_item_id"
    ...
    Given set the request headers, query params and payload with json file name "ordernotify.json"
      ...
      | application/json | order_id | cart_item_id | order_id          | txn_amount     | 1235272705          |
    Then perform the "POST" api call 5 times and validate response code 200
